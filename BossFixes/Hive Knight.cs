...
namespace PantheonOfRegions.Behaviours
{
    internal class HiveKnight : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                _dropper.transform.GetChild(i).gameObject.LocateMyFSM("Control").Fsm.GetFsmFloat("X Left").Value = 15f;
                _dropper.transform.GetChild(i).gameObject.LocateMyFSM("Control").Fsm.GetFsmFloat("X Right").Value = 35f;
            }
            ...
            _glob.transform.GetChild(0).gameObject.LocateMyFSM("Control").Fsm.GetFsmVector3("Tween Vector").Value = new Vector2(0f, 3f);
            _glob.transform.GetChild(1).gameObject.LocateMyFSM("Control").Fsm.GetFsmVector3("Tween Vector").Value = new Vector2(0f, 5f);
            _glob.transform.GetChild(2).gameObject.LocateMyFSM("Control").Fsm.GetFsmVector3("Tween Vector").Value = new Vector2(0f, 7f);
